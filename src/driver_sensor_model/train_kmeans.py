...
for K in tqdm(Ks):
	...
	p_a_m_occ = np.zeros((K, batch_y.shape[1]))
	p_a_m_free = np.zeros((K, batch_y.shape[1]))

	for i in range(batch_y.shape[1]):
		p_m_occ = np.sum(batch_y[:,i] == 1).astype(float)/batch_y.shape[0]
		p_m_free = np.sum(batch_y[:,i] == 0).astype(float)/batch_y.shape[0]

		for k in range(K):
			p_a_m_occ[k,i] = np.sum((cluster_ids_x == k) & (batch_y[:,i] == 1)).astype(float)/batch_y.shape[0]
			p_a_m_free[k,i] = np.sum((cluster_ids_x == k) & (batch_y[:,i] == 0)).astype(float)/batch_y.shape[0]

		if p_m_occ != 0:
			p_a_m_occ[:,i] = p_a_m_occ[:,i]/p_m_occ
		else:
			p_a_m_occ[:,i] = p_a_m_occ[:,i]/1.0

		if p_m_free != 0:
			p_a_m_free[:,i] = p_a_m_free[:,i]/p_m_free
		else:
			p_a_m_free[:,i] = p_a_m_free[:,i]/1.0

	p_m_a = p_a_m_occ/(p_a_m_occ + p_a_m_free)
	p_m_a_np = p_m_a

	pkl.dump([model, p_m_a_np], open(os.path.join(dir_models, "clusters_kmeans_K_" + str(K) + ".pkl"), "wb" ) )

	cluster_centers_np = cluster_centers
	cluster_centers_np = np.reshape(cluster_centers_np, (K,nt,-1))
	...
	if plot_train:
		...
		picture_file = os.path.join(dir_models, 'state_clusters.png')
...
if plot_train:
	...
	plt.ylabel('Accuracy')
	plt.savefig(os.path.join(dir_models, 'acc_clusters.png'))